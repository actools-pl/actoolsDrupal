...
#
# Pins the LIVE (A) backup-artifact contract at baseline c17197f: the daily cron
# generator (modules/backup/cron.sh -> /etc/cron.daily/actools-backup) and the
# `restore` consumer (cli/actools) must name the SAME artifact. The agreement is
# four-fold — same root (${INSTALL_DIR}/backups), same filename stem (_db_), same
# extension (.sql.gz), same checksum-sidecar convention (<artifact>.sha256) — plus
# the integrity-or-delete sidecar the producer writes. See
# docs/backup-format-contract.md for the canonical scheme and the divergence ledger.
#
# This guard pins ONLY the live (A) agreement. It deliberately asserts NOTHING about
# the encrypted (B, encrypted_backup.sh) or PITR (C, db-full-backup.sh /
# pitr-restore.sh) drafts — those are not wired (TARGET / NOT YET LIVE); E2 and E3
# each extend this guard when they wire their variant.
#
...
#     by the two non-vacuity arms (which expect it to FAIL on a doctored copy);
...
@test "non-vacuous: a restore glob that drifts (_db_ -> _database_) FAILS agreement" {
  ...
  cp "$CLI" "$doctored_cli"
  # Surgical: only the restore arm's "${env}"_db_ glob; the restore-test arm's
  # prod_db_*.sql.gz glob (no "${env}" prefix) is intentionally left untouched.
  sed -i 's/"\${env}"_db_/"\${env}"_database_/' "$doctored_cli"
