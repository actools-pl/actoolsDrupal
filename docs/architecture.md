...
│
├── cron/                   scheduled jobs (stats; backup schedule generated by modules/backup/cron.sh)
├── certs/                  TLS certificates (private keys gitignored)
├── templates/              Dockerfiles, CI workflows, settings.php template
├── tests/                  bats suite — guards, generated golden-drift, contracts, real-install e2e
