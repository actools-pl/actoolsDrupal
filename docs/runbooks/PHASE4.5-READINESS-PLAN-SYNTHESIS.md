...
| **C4** | Disentangle the overloaded "Phase" vocabulary | New `PHASE_VOCABULARY.md`; annotate the `CHANGELOG v10.0` "Phase 1 = 32 modules complete" as historical-marketing; fix the `technical-roadmap.md` "at a glance" tree. **No rewriting of historical ledger entries.** | Low | Grep-gate: no doc asserts "Phase 1 complete" without a historical qualifier; doc-link check | S |
